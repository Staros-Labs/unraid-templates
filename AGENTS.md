...
- Tracked changes, commit/push, shared Git connector actions, deploys, destructive operations, and unknown mutating MCP tools require the repository's Linear bootstrap flow and a live worktree lease.
...
- Native pathless worker dispatch requires active parent worktree context. Resolver accepts only active `.codex-session-active` plus live `.agent-session`; parked, expired, legacy, missing, or ambiguous state blocks. Re-enter or resume exact context; never bypass guard.
...
- Linear lifecycle, comments, and coordination writes may run without a lease. Credential and other destructive coordination actions remain lease-gated.
- Known read-only connector variants may run pre-lease, including GitHub fetch tools, `mcp__codegraph__codegraph_explore`, `web__run`, and `request_user_input`; connector names with explicit mutation verbs remain gated.
- Pathless connector and app mutations use current checkout context. Guard does not infer a worktree from task or session identity, and shared Git connector actions require this checkout's live lease.
...
- Bounded `&&`, `||`, and semicolon chains are lease-free unless a segment explicitly mutates repository, shared source-control, coordination, deployment, or production state. Substitution and other ambiguous shell syntax remain coordinated.
...
- Merge, release, deploy-wrapper, and production HTTP mutations remain blocked during degradation unless `scripts/linear-degraded-approval.sh consume` consumes an exact single-use mode-600 receipt bound to repository, branch, action, target, operator, reason, and expiry. Direct local and known-fleet host lifecycle commands remain outside lease and receipt gating. Environment-variable bypasses are forbidden.
...
- During a confirmed Linear outage, `scripts/rollout-linear-enrollment.sh --git-only --operator <name>` remains limited to enrollment-only rollout paths. General degraded execution uses the local outbox and still requires Agent Coordination; it never authorizes merge, release, deploy, or production work without a single-use receipt.
